...
AddReplicableComponent("bogd_com_rpc_event")  --- RPC通道 模块
AddReplicableComponent("bogd_com_level_sys")  --- 等级模块

AddReplicableComponent("bogd_com_acceptable")  --- 通用接受模块
...
AddReplicableComponent("bogd_com_map_jumper")  --- 地图跳跃


--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
---

    if EntityScript.ReplicateComponent_bogd_old_fn == nil then

        -- EntityScript.ReplicateComponent
        EntityScript.ReplicateComponent_bogd_old_fn = EntityScript.ReplicateComponent
        EntityScript.ReplicateComponent = function(self,name)
            self.ReplicateComponent_bogd_old_fn(self,name)
            local replica_com = self.replica[name] or self.replica._[name]
            if replica_com then
                self:PushEvent("BOGD_OnEntityReplicated."..tostring(name),replica_com)
