...
---
    ...
    --- 基础的文件读写函数
    local fileName = "book_of_gods_and_demons.text" -- 文件名缓存
...
function TUNING.BOGD_FN:Get_Cross_Archived_Data_By_userid(userid)
    return Get_Cross_Archived_Data_By_userid(userid) or {}
end
function TUNING.BOGD_FN:Set_Cross_Archived_Data_By_userid(userid,_table) 
    Set_Cross_Archived_Data_By_userid(userid,_table) 
end
function TUNING.BOGD_FN:Set_ThePlayer_Cross_Archived_Data(index,value)
    if ThePlayer and ThePlayer.userid then
        local all_data = TUNING.BOGD_FN:Get_Cross_Archived_Data_By_userid(ThePlayer.userid) or {}
        all_data[index] = value
        TUNING.BOGD_FN:Set_Cross_Archived_Data_By_userid(ThePlayer.userid,all_data)
    end
end
function TUNING.BOGD_FN:Get_ThePlayer_Cross_Archived_Data(index)
    if ThePlayer and ThePlayer.userid then
        local all_data = TUNING.BOGD_FN:Get_Cross_Archived_Data_By_userid(ThePlayer.userid)
