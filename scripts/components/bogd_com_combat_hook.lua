...
----------------------------------------------------------------------------------------------------------------------------------
local bogd_com_combat_hook = Class(function(self, inst)
    self.inst = inst
...
---------------------------------------------------------------------------------------------------
----
    function bogd_com_combat_hook:Set(tar_inst,fn)
        self:Add(tar_inst,fn)
    end
    function bogd_com_combat_hook:Add(tar_inst,fn)
        self.modifiler_fns[tar_inst] = fn
        tar_inst:ListenForEvent("onremove",self.event_in_tar_inst)
    end
    function bogd_com_combat_hook:Remove(tar_inst)
        local new_table = {}
