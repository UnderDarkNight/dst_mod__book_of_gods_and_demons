...
----------------------------------------------------------------------------------------------------------------------------------------------------
---
    local prefab_name = "bogd_item_demonization_pill"

    local assets = {
        Asset("ANIM", "anim/bogd_item_demonization_pill.zip"), 
        Asset( "IMAGE", "images/inventoryimages/bogd_item_demonization_pill.tex" ),  -- 背包贴图
        Asset( "ATLAS", "images/inventoryimages/bogd_item_demonization_pill.xml" ),
    }
------------------------------------------------------------------------------------------------------
--- workable setup
    local function workable_setup(inst)
        ...
        if TheWorld.ismastersim then
            ...
            inst.components.bogd_com_workable:SetOnWorkFn(function(inst,doer)
                if doer.replica.bogd_com_level_sys:IsHuman() then
                    doer.components.bogd_com_level_sys:OnBecomeDemon()
                end
                inst:Remove()
                return true
            end)
...
local function fn()
    ...
    -- inst.AnimState:SetBank("bogd_item_demonization_pill") -- 地上动画
    -- inst.AnimState:SetBuild("bogd_item_demonization_pill") -- 材质包，就是anim里的zip包
